...
現在公開している動作確認済みセットは **v0.8.4のみ** です。

## どちらを使いますか？

### そのまま使いたい方

[v0.8.4 Release](https://github.com/SuperGrave/chill-with-kiritan/releases/tag/v0.8.4) から、用途に合うファイルを入手してください。

- `Chill-with-Kiritan-v0.8.4-release.zip`: Wallpaper Engine用データ、Companion、説明書の全部入り
- `Chill-with-Kiritan-WallpaperEngine-v0.8.4.zip`: Wallpaper Engine用データのみ
- `Tohoku.Companion_0.8.4_x64-setup.exe`: Companionインストーラー
...
このリポジトリの `main`、またはv0.8.4 Releaseの `Chill-with-Kiritan-v0.8.4-source.zip` が制作版です。コンパイル済みアプリ、再配布できないVRM/VRMA、APIキー、Spotify認証情報、個人設定、個人用原稿は含みません。
