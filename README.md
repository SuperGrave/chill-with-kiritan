...
現在の開発本流は **v0.8.4**。以前 `v4` と呼んでいた積み上げを
`v0.8.0` として扱い、以後の小さな更新は `v0.8.1`, `v0.8.2`, `v0.8.3`, `v0.8.4` のように
v0.8 系で進める。`v2` / `v3` など過去の版名はレガシー版としてそのまま残す。

Git上の現役開発ラインは `main`。節目は `v0.8.0`, `v0.8.1`, `v0.8.4` のようなtagで残し、
...
出力先は `release/v0.8.4/`。このフォルダは生成物なのでGit管理しない。
次の小更新は `-Version 0.8.5` のように指定する。

- `release/v0.8.4/wallpaper-engine/Chill with Kiritan/`: Wallpaper Engineへ取り込むフォルダ
- `release/v0.8.4/Chill-with-Kiritan-WallpaperEngine-v0.8.4.zip`: 共有用zip
- `release/v0.8.4/companion/`: Companionのexe/installer
- `release/v0.8.4/Chill-with-Kiritan-v0.8.4-release.zip`: まとめzip

配布物を初めて使う人向けの短い説明は `README_DISTRIBUTION_JP.md` を参照。
