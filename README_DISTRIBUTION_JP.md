...
初回は「1920×1080用サンプル」が適用されています。1920×1080・1920×1200には通常版と「スペクトラム含」版の合計4プリセットを同梱しています。4Kでは `STUDIO（調律）` の画面サイズから `3840x2160` を選んで調整・保存してください。
...
- イヤホン接続後にBPMが止まった: v0.9.2はWindowsの既定出力変更を検知して自動再接続します。数秒待っても戻らない場合は、Windowsで再生先が意図した機器になっているか確認してください。
