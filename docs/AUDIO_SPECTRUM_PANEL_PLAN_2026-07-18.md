...
今回の対象はパネル表示、信号処理、BPM推定、同期イベントまで。**キリタンのリズム連動（ビートに合わせた首ノッド等のモーション）は次段階**とし、今回はモーションコードから独立したイベント契約までを仕込む。
...
- BPM: `showBpm`（既定ON）、`bpmLockSeconds`（既定5秒、2〜12秒）
...
- `kiritan:audio-bpm-sync.detail`: `bpm`, `detectedBpm`, `confidence`, `stableForMs`, `lockedAt`, `source`。
- `kiritan:audio-beat.detail`: `energy`, `at`, `source`, `detectedBpm`, `lockedBpm`。Claude側は `lockedBpm !== null` の間だけノッドへ利用できる。
- 無音または大きなテンポ変更でロックは解除される。再ロック時は同じBPMでも新しい同期イベントを発火する。
- イベントを受け取るだけで、現段階ではVRM、モーション、ディレクター状態を変更しない。

## 10. 次段階（キリタン連動 — 今回は対象外）

1. `kiritan:audio-bpm-sync` で同期セッションを開始し、`kiritan:audio-beat` を壁紙側（VrmViewer）で購読して首の微小ノッドへ反映。
2. `music_listen` モード実装時に、ビート強度をアンビエント抽選の重み（ノリの深さ）へ反映。
3. Spotify再生中メタデータ（既存 `/api/ui`）と組み合わせ、「曲が流れている間だけノる」ゲート。

## 11. 追加検証結果

- 数値テスト: 49 checks PASS。
- Overlay / Companion production build: PASS。
- devモック実測: 約120 BPMを `BPM 121` として検知し、`DETECTING` 進捗後に `KIRITAN SYNC` へ遷移。
- VRM・モーション・ディレクター側のファイル変更なし。
