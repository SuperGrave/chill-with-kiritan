...
- 監査基準HEAD: `e6016df` + 2026-07-02継続差分（branch `feat/pose-composer-0.8`。Stage A-D時点は`f0f4fb9`、Stage F優先度1-4時点は`e6016df`）
...
結論: **「壁紙が生活しているように見える」ラインの最後の壁（Director非稼働・Companion連携404）は解消済み**。続けてCompanion運用品質の前半7項目（single-instance・port競合表示・tray完全終了・保存の安全性・loopback固定試験・mutating API token・secrets分離）も実装した。残る大物はWallpaper Engine実機package化とCompanion運用品質の後半3項目（connection status・自動起動・export/import）。
...
| Companion | PARTIAL | PARTIAL（kiritan受信ルート、single-instance、tray終了、atomic save、mutating API token、secrets分離を追加） | |
...
| 01 KiritanPoster | `POST http://127.0.0.1:40313/api/kiritan/state` + `X-Companion-Token` | 03 Companion | ✅ WORKING | Directorが稼働中、毎フレーム`maybePost()`呼び出し（実送信はmode変化時+30s heartbeat）。tokenは`GET /api/auth/token`で取得・キャッシュ | Companion不在→fetch失敗をfire-and-forgetで握り潰す（`onError`のみ、壁紙は無停止） | なし（実装済み・実機E2Eはtoken導入前、token後は統合テスト+poster HTTP testで確認） |
...
| 03 Companion | `GET/PUT /api/ui`, `POST /api/presets/*` | 02 UI Overlay | ✅ WORKING（token導入後もbuild/統合テスト確認済み） | mutating requestは`X-Companion-Token`必須。overlayは`GET /api/auth/token`で取得・キャッシュ | GET系pollingはtoken不要 | |
...
|---|---|
| `cargo check` | PASS |
| `cargo test` | PASS（unit 9 + integration 4 = 13/13。`kiritan_state_post_and_get`、token必須化、CORS origin判定、loopback固定試験、secrets分離/migration試験を含む） |
...
8. **Companionの最低限の運用品質** → **一部完了**: single-instance / port bind失敗UI / tray完全終了 / atomic write+backup / loopback固定試験 / mutating API token / secrets分離の7項目は実装済み。残りはconnection status・自動起動・export/import（[実行計画§4](COMPLETION_EXECUTION_PLAN_2026-07-01.md#4-companion運用品質の計画)参照、優先度8-10）
9. **配布対象を確定する** → 未着手
...
- `03_companion/src-tauri/src/api.rs`, `state.rs`, `models.rs`, `tests/api_test.rs`（kiritan state route、loopback固定、mutating API token）
- `03_companion/src/api.ts`, `02_ui-overlay/src/services/companionClient.ts`, `01_wallpaper/src/lib/motion/director/kiritanPoster.ts`（tokenヘッダ付与）
- `tools/test_daypart.mjs`（新規）
- `docs/POSE_COMPOSER_0_8_PROGRESS.md`（Stage 0-4 green + park記録）

## 11. 次の実装順（推奨・更新）

1. ~~Companion運用品質 優先度1-7（single-instance → port競合UI → tray完全終了 → atomic save+backup → loopback固定試験 → mutating API token → secrets分離）~~ → **完了**
2. **Companion運用品質 優先度8-10（MUST #8残り）**: connection status → 自動起動 → export/import。次はCompanion状態タブで`GET /api/kiritan/state`のfreshnessを表示する。
3. **Wallpaper Engine実機package（MUST #7）**: 専用project folder、`project.json`、`wallpaperPropertyListener`（FPS/pause）。実機導入試験は目視待ちとして明確に区別する。
4. **背景アート3枚の調達 or 差し替え判断（MUST #6残り）**: 自作/CC0調達/据え置きのいずれかを確定。
5. **配布対象確定（MUST #9）**: 個人利用限定か公開配布かで、モデル差し替え導線・font license・CC-BY creditsの要否が変わる。
