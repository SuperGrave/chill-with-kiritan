...
結論: **「壁紙が生活しているように見える」ラインの最後の壁（Director非稼働・Companion連携404）は解消済み**。続けてCompanion運用品質の前半6項目（single-instance・port競合表示・tray完全終了・保存の安全性・loopback固定試験・mutating API token）も実装した。残る大物はWallpaper Engine実機package化とCompanion運用品質の後半4項目（secrets分離・connection status・自動起動・export/import）。
...
| Companion | PARTIAL | PARTIAL（kiritan受信ルート、single-instance、tray終了、atomic save、mutating API tokenを追加） | |
...
|---|---|
| `cargo check` | PASS |
| `cargo test` | PASS（unit 7 + integration 4 = 11/11。`kiritan_state_post_and_get`、token必須化、CORS origin判定、loopback固定試験を含む） |
...
8. **Companionの最低限の運用品質** → **一部完了**: single-instance / port bind失敗UI / tray完全終了 / atomic write+backup / loopback固定試験 / mutating API tokenの6項目は実装済み。残りはsecrets分離・connection status・自動起動・export/import（[実行計画§4](COMPLETION_EXECUTION_PLAN_2026-07-01.md#4-companion運用品質の計画)参照、優先度7-10）
9. **配布対象を確定する** → 未着手
...
1. ~~Companion運用品質 優先度1-6（single-instance → port競合UI → tray完全終了 → atomic save+backup → loopback固定試験 → mutating API token）~~ → **完了**
2. **Companion運用品質 優先度7-10（MUST #8残り）**: secrets分離 → connection status → 自動起動 → export/import。次はsecrets分離（旧`companion-data.json`内`secrets`から`secrets.json`へ自動migration）から着手する。
