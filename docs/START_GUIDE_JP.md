...
1. エクスプローラーのアドレス欄に `%APPDATA%\tohoku-companion\personal_news_scripts` と入力して開きます（無ければフォルダを作成します）。
2. UTF-8 の `.txt` ファイルとして原稿を置きます。1行がティッカーの1表示になり、行の長さに応じて表示時間は自動調整されます。
3. Companion の `CONTENT（中身）> 個人ニュース` で原稿を選び、再生ボタンで開始します。章送り・一時停止・リピートも操作できます。
