...
2. UTF-8 の `.txt` ファイルとして原稿を置きます。物理的な1行に複数の日本語文があっても、読み込み時に `。` `！` `？` の切れ目で1表示ずつへ自動変換され、文の長さに応じて表示時間が調整されます。元ファイルは書き換えません。
3. Companion の `CONTENT（中身）> 個人ニュース` で原稿を選び、再生ボタンで開始します。行送り・章送り・一時停止・リピートも操作でき、現在の `LINE` 番号も確認できます。
